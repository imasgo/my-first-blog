...
class HistoryNoteForm(forms.ModelForm):
    ...
    family_relationship = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Семейные отношения'}))
    others = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Другое'}))
    sources = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Источники'}))
    literature = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Литература'}))
